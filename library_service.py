...
class AudioBookShelfLibraryService:
	...
	def get_library_items(self, library_id, limit=None, page=None, sort=None, desc=None, filter=None, minified=None, collapseseries=None, include=None):
		...
		xbmc.log(f"response: {response.json()}", xbmc.LOGINFO)
		return response.json()
	...
	def get_file_url(self, iid):
		...
		if "audioTracks" in response and len(response["audioTracks"]) > 0:
			for audiotrack in response["audioTracks"]:
				relative_content_url = audiotrack["contentUrl"]
				full_content_url.append({
        			"url" : f"{self.base_url}{relative_content_url}?token={self.token}", 
                    "title" : audiotrack["title"],
                    "duration" : audiotrack["duration"],
                    "startOffset" : audiotrack["startOffset"],
                    "index" : audiotrack["index"] - 1
                    })
    
		if not full_content_url or len(full_content_url) < 1:
	...
	def update_media_progress(self, library_item_id, data, episode_id=None):
		...
		if episode_id:
			endpoint += f"/{episode_id}"
		xbmc.log(str(data), xbmc.LOGINFO)
		response = requests.patch(self.base_url + endpoint, headers=self.headers, json=data)
		response.raise_for_status()
		if response.status_code != 200:
			xbmc.log(f"Patch Media Progress Response: {response.status_code})", xbmc.LOGDEBUG)
	...
	def get_start_time(self, library_item_id):
		item = self.play_library_item_by_id(library_item_id)
